...
随机游走很简单，从一个节点$v_i$（这里也记作$\mathcal{W}_{v_i}^1$）出发，每次都随机跳转到下一个节点，一直走直到经过了t个节点，产生一个序列$\mathcal{W}_{v_i}^1, \mathcal{W}_{v_i}^2, \ldots, \mathcal{W}_{v_i}^t$就是随机游走序列$\mathcal{W}_{v_i}$。

随机游走的好处

* 保留了原始图中的结构信息。比如社交网络、自然产生的图一般都服从幂律分布，随机游走访问点的次数也是幂律分布。
* 可以并行。
...
一种传统的想法是用随机游走的前后总共i-1个值来预测第i个值，目标是使如下面的公式所示的概率尽量大。
